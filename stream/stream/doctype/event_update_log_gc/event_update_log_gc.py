...
class EventUpdateLogGC(Document):
	def after_insert(self):
		...
		enqueued_method = (
			"stream.stream.doctype.event_consumer_gc.event_consumer_gc.notify_event_consumers"
		)
		jobs = get_jobs()
...
def is_consumer_uptodate(update_log, consumer):
	...
	prev_log_consumers = frappe.get_all(
		"Event Update Log Consumer GC",
		fields=["consumer"],
		filters={
			"parent": prev_logs[0].name,
			"parenttype": "Event Update Log GC",
...
def mark_consumer_read(update_log_name, consumer_name):
	...
	"""
	update_log = frappe.get_doc("Event Update Log GC", update_log_name)
	if len([x for x in update_log.consumers if x.consumer == consumer_name]):
		return

	frappe.get_doc(
		frappe._dict(
			doctype="Event Update Log Consumer GC",
...
def get_unread_update_logs(consumer_name, dt, dn):
	...
	already_consumed = [
		...
		SELECT
			update_log.name
		FROM `tabEvent Update Log GC` update_log
		JOIN `tabEvent Update Log Consumer GC` consumer ON consumer.parent = %(log_name)s
		WHERE
...
def get_update_logs_for_consumer(event_consumer, doctypes, last_update):
	...
	from stream.stream.doctype.event_consumer_gc.event_consumer_gc import has_consumer_access

	consumer = frappe.get_doc("Event Consumer GC", event_consumer)
