...
class EventConsumerGC(Document):
	...
	def clear_cache(self):
		from stream.stream.doctype.event_update_log_gc.event_update_log_gc import (
	...
	def on_trash(self):
		for i in frappe.get_all("Event Update Log Consumer GC", {"consumer": self.name}):
			frappe.delete_doc("Event Update Log Consumer GC", i.name)

	def update_consumer_status(self):
		consumer_site = get_consumer_site(self.callback_url)
		event_producer = consumer_site.get_doc("Event Producer GC", get_url())
		...
		for entry in config:
			if entry.get("has_mapping"):
				ref_doctype = consumer_site.get_value(
					"Document Type Mapping GC", "remote_doctype", entry.get("mapping")
				).get("remote_doctype")
			else:
				ref_doctype = entry.get("ref_doctype")

			entry["status"] = frappe.db.get_value(
				"Event Consumer Document Type GC", {"parent": self.name, "ref_doctype": ref_doctype}, "status"
			)
...
def register_consumer(data):
	...
	if frappe.db.exists("Event Consumer GC", data["event_consumer"]):
		return None
	...
	consumer = frappe.new_doc("Event Consumer GC")
...
def get_consumer_site(consumer_url):
	...
	consumer_doc = frappe.get_doc("Event Consumer GC", consumer_url)
...
def get_last_update():
	...
	updates = frappe.get_list(
		"Event Update Log GC", "creation", ignore_permissions=True, limit=1, order_by="creation desc"
	)
	if updates:
...
def notify_event_consumers(doctype):
	...
	event_consumers = frappe.get_all(
		"Event Consumer Document Type GC", ["parent"], {"ref_doctype": doctype, "status": "Approved"}
	)
	for entry in event_consumers:
		consumer = frappe.get_doc("Event Consumer GC", entry.parent)
...
def notify(consumer):
	...
	if consumer_status == "online":
		try:
			...
			client.post_request(
				{
					"cmd": "stream.stream.doctype.event_producer_gc.event_producer_gc.new_event_notification",
	...
	if not consumer.flags.notified:
		enqueued_method = "stream.stream.doctype.event_consumer_gc.event_consumer_gc.notify"
		jobs = get_jobs()
...
def has_consumer_access(consumer, update_log):
	...
	if isinstance(consumer, str):
		consumer = frappe.get_doc("Event Consumer GC", consumer)
	...
	if not frappe.db.exists(update_log.ref_doctype, update_log.docname):
		...
		last_update_log = frappe.get_doc("Event Update Log GC", last_update_log[0].name)
