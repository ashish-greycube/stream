...
class EventProducerGC(Document):
	...
	def on_update(self):
		if not self.incoming_change:
			if frappe.db.exists("Event Producer GC", self.name):
	...
	def on_trash(self):
		last_update = frappe.db.get_value("Event Producer Last Update GC", dict(event_producer=self.name))
		if last_update:
			frappe.delete_doc("Event Producer Last Updat GC", last_update)
	...
	def create_event_consumer(self):
		...
		if self.is_producer_online():
			...
			response = producer_site.post_api(
				"stream.stream.doctype.event_consumer_gc.event_consumer_gc.register_consumer",
	...
	def set_last_update(self, last_update):
		last_update_doc_name = frappe.db.get_value(
			"Event Producer Last Update GC", dict(event_producer=self.name)
		)
		if not last_update_doc_name:
			frappe.get_doc(
				dict(
					doctype="Event Producer Last Update GC",
			...
			frappe.db.set_value(
				"Event Producer Last Update GC", last_update_doc_name, "last_update", last_update
			)

	def get_last_update(self):
		return frappe.db.get_value(
			"Event Producer Last Update GC", dict(event_producer=self.name), "last_update"
		)
	...
	def get_request_data(self):
		...
		for entry in self.producer_doctypes:
			if entry.has_mapping:
				...
				dt = frappe.db.get_value("Document Type Mapping GC", entry.mapping, "remote_doctype")
			else:
	...
	def update_event_consumer(self):
		if self.is_producer_online():
			...
			event_consumer = producer_site.get_doc("Event Consumer GC", get_url())
				...
				for entry in self.producer_doctypes:
					if entry.has_mapping:
						...
						ref_doctype = frappe.db.get_value(
							"Document Type Mapping GC", entry.mapping, "remote_doctype"
						)
					else:
...
def get_producer_site(producer_url):
	...
	producer_doc = frappe.get_doc("Event Producer GC", producer_url)
	producer_site = FrappeClient(
		url=producer_url,
...
def pull_producer_data():
	...
	if response.status_code == 200:
		for event_producer in frappe.get_all("Event Producer GC"):
...
def pull_from_node(event_producer):
	...
	event_producer = frappe.get_doc("Event Producer GC", event_producer)
		...
		if entry.status == "Approved":
			if entry.has_mapping:
				(mapped_doctype, mapping) = frappe.db.get_value(
					"Document Type Mapping GC", entry.mapping, ["remote_doctype", "name"]
				)
...
def sync(update, producer_site, event_producer, in_retry=False):
	...
	try:
		...
		if update.update_type in ["Create","Update"]:
			...
			payload = {
				"custom_gov_creation":"{0}".format(creation_details.get("creation")),
				"custom_gov_modified":"{0}".format(creation_details.get("modified"))
			}
...
def get_updates(producer_site, last_update, doctypes):
	...
	docs = producer_site.post_request(
		{
			"cmd": "stream.stream.doctype.event_update_log_gc.event_update_log_gc.get_update_logs_for_consumer",
			"event_consumer": get_url(),
...
def log_event_sync(update, event_producer, sync_status, error=None):
	...
	doc = frappe.new_doc("Event Sync Log GC")
...
def get_mapped_update(update, producer_site):
	...
	mapping = frappe.get_doc("Document Type Mapping GC", update.mapping)
	if update.update_type == "Create":
...
def new_event_notification(producer_url):
	...
	enqueued_method = "stream.stream.doctype.event_producer_gc.event_producer_gc.pull_from_node"
	jobs = get_jobs()
...
def resync(update):
	...
	event_producer = frappe.get_doc("Event Producer GC", update.event_producer)
	if update.mapping:
		update = get_mapped_update(update, producer_site)
		update.data = json.loads(update.data)
